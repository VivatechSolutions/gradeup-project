...
export default function SlideEditorPage() {
  ...
  const previewSlide = deck?.proposal?.slideId === slide?.id ? deck.proposal.preview.slide : null;
  ...
  return <main className={`se-app ${presenting ? 'se-presenting' : ''}`}>
    {!presenting && <>
      ...
      <Tool label="Present" disabled={!!deck.proposal} onClick={() => { setPresenting(true); window.document.documentElement.requestFullscreen?.().catch(() => {}); }}><Play size={18} /></Tool>
    ...
    <div className="se-workspace">
      {!presenting && <nav className="se-slides" aria-label="Slides"><div className="se-inline"><Tool label="Add slide" disabled={!editable || doc.slides.length >= 50} onClick={() => addSlide()}><Plus size={18} /></Tool><Tool label="Duplicate slide" disabled={!editable || doc.slides.length >= 50} onClick={() => addSlide(true)}><Copy size={16} /></Tool><Tool label="Delete slide" disabled={!editable || doc.slides.length < 2} onClick={removeSlide}><Trash2 size={16} /></Tool></div>{doc.slides.map((s, i) => { const thumbnailSlide = deck.proposal?.slideId === s.id && deck.proposal.preview.slide ? deck.proposal.preview.slide : s; return <button key={s.id} className={`se-thumbnail ${s.id === slide.id ? 'active' : ''}`} aria-label={`Slide ${i + 1}: ${s.title}`} aria-current={s.id === slide.id ? 'true' : undefined} draggable={editable} onDragStart={() => { dragged.current = s.id; }} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); if (!editable) return; const from = dragged.current; change(d => { const source = d.slides.findIndex(v => v.id === from), target = d.slides.findIndex(v => v.id === s.id); if (source >= 0) d.slides.splice(target, 0, d.slides.splice(source, 1)[0]); }); }} onClick={() => chooseSlide(s.id)}><div className="se-thumbnail-canvas"><SlideCanvas slide={thumbnailSlide} deckId={deckId} token={shareToken} scale={.12} readOnly /></div><span>{i + 1}. {s.title || 'Untitled'}</span></button>; })}</nav>}
      <section className="se-center"><div className="se-stage-space" ref={viewport} style={{ position: 'relative' }}>{previewing && !presenting && <div role="status" style={{ position: 'absolute', top: 10, left: '50%', transform: 'translateX(-50%)', zIndex: 3, padding: '6px 10px', borderRadius: 5, background: '#173d2e', color: '#fff', fontSize: 11, fontWeight: 700, boxShadow: '0 2px 8px #17251f33', pointerEvents: 'none' }}>AI Preview · Not saved</div>}<SlideCanvas slide={displayedSlide} deckId={deckId} token={shareToken} scale={scale} selected={presenting || previewing ? [] : selected} onSelect={ids => useEditor.setState({ selected: ids })} onChange={elements => { if (editable) modifyElements(() => elements); }} readOnly={!editable || presenting} snap={snap} /></div>
        {!presenting && <><div className="se-selection-tools">{([['x','start',AlignLeft],['x','center',AlignCenter],['x','end',AlignRight],['y','start',AlignStartVertical],['y','center',AlignCenterVertical],['y','end',AlignEndVertical],['x','distribute',GalleryHorizontalEnd],['y','distribute',GalleryVerticalEnd]] as const).map(([axis, mode, Icon]) => <Tool key={axis + mode} label={`${mode} ${axis === 'x' ? 'horizontally' : 'vertically'}`} disabled={!editable || !selected.length} onClick={() => align(axis, mode)}><Icon size={16} /></Tool>)}<span>Slide {doc.slides.indexOf(slide) + 1} of {doc.slides.length}</span>{selectedElement?.type === 'image' && <button disabled={!editable} onClick={() => { replacing.current = true; file.current?.click(); }}>Replace image</button>}</div><label className="se-notes">Speaker notes<textarea aria-label="Speaker notes" disabled={!editable} maxLength={20000} value={displayedSlide.notes} onChange={e => change(d => { d.slides.find(v => v.id === slide.id)!.notes = e.target.value; })} /></label></>}
      </section>
