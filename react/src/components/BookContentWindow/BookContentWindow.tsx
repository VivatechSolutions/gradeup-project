...
function serializeEnrichedForGenius(enrichedContent: any, fallbackTitle: string) {
  ...
  const pushAvatarExplanation = (value: any) => {
    ...
    segments.forEach((segment: any) => {
      ...
      if (segmentType === "flashcard") return;

      const text = flattenContentToText(segment?.text || segment?.content || "").trim();
      if (!text) return;
      lines.push(text);
    });
  };
...
function extractSectionTopicsFromContent(content: any, chapterId: string | number) {
  const topics: Array<{ id: string; label: string; title: string; number?: string | null; anchor: string; sectionType?: string | null }> = [];
    ...
    if (title && isReaderTopic(title)) {
      ...
      if (!seen.has(key)) {
        ...
        topics.push({
          ...
          sectionType: value.type || value.kind || value.section_type || value.sectionType || null,
        });
      }
    }
...
function buildStructuredLayout(content: any, chapterId: string | number, topicMap: Map<string, string>) {
  ...
  const visit = (value: any) => {
      ...
      value.sections.forEach((section: any) => {
        ...
        if (title && isReaderTopic(title)) {
          ...
          blocks.push({
            ...
            content: label,
            anchor,
            sectionType: section.type || section.kind || section.section_type || section.sectionType || null,
          });
        }
    ...
    [value.children, value.items, value.topics, value.enrichment, value.sub_items].forEach(visit);
...
function buildChapterFromUnit(unit: any, contentPayload: any, index: number) {
  ...
  const sectionTopics = (contentPayload?.sectionTopics || extractSectionTopicsFromContent(enrichedContent || contentPayload, unit.id)).map((topic: any) => ({
    ...
    sectionType: topic.sectionType || topic.section_type || topic.type || null,
  }));
...
const BookContentWindow = () => {
  ...
  const [avatarStatus,      setAvatarStatus]      = useState<"idle" | "playing" | "waiting_flashcard" | "paused" | "completed">("idle");
  const [avatarError,       setAvatarError]       = useState<string | null>(null);
  const [avatarFlashcards,  setAvatarFlashcards]  = useState<Record<string, AvatarFlashcard>>({});
  const [avatarGeneratingCards, setAvatarGeneratingCards] = useState(false);
  const [avatarMcqAnswers,  setAvatarMcqAnswers]  = useState<Record<string, string>>({});
  const [avatarMcqFeedback, setAvatarMcqFeedback] = useState<Record<string, { correct: boolean; message: string }>>({});
  const [avatarDoubt,       setAvatarDoubt]       = useState("");
  const [avatarClarifications, setAvatarClarifications] = useState<Array<{ text: string; emotion?: string }>>([]);
  ...
  const sectionTopicByAnchor = new Map(
    (displayChapter?.sectionTopics || []).map((topic: any) => [topic.anchor, topic]),
  );
  const visiblePageStartIndex = isSinglePageView ? safeSpreadIndex : safeSpreadIndex * 2;
  ...
  const avatarCurrentCard = avatarCurrentSegment
    ? avatarFlashcards[avatarCurrentKey] || avatarFlashcards[String(avatarCurrentSegment.segment_id || "")]
    : null;
  const avatarDisplaySegment = avatarCurrentCard
    ? { ...avatarCurrentSegment, ...avatarCurrentCard }
    : avatarCurrentSegment;
  const avatarSegmentType = String(avatarDisplaySegment?.type || "").toLowerCase();
  const avatarFlashcardType = String(
    avatarDisplaySegment?.flashcard_type ||
      (avatarDisplaySegment?.question || avatarDisplaySegment?.options ? "mcq" : "informative"),
  ).toLowerCase();
  const avatarIsMcq =
    avatarSegmentType === "flashcard" &&
    (avatarFlashcardType === "mcq" || Boolean(avatarDisplaySegment?.question));
  const avatarCanAnswerMcq =
    avatarIsMcq && avatarStatus === "waiting_flashcard" && !avatarMcqFeedback[avatarCurrentKey];
  ...
  async function completeAvatarSession() {
    ...
    try {
      ...
      setAvatarError(error?.message || "Unable to end the avatar session cleanly.");
    }
  }
  ...
  async function generateAvatarCardsForSession(sessionId: string, segments: AvatarSegment[]) {
    ...
    try {
      ...
      const cards = Array.isArray(response?.flash_cards) ? response.flash_cards : [];
      ...
      setAvatarError(error?.message || "Some avatar flashcards could not be generated.");
    } finally {
      setAvatarGeneratingCards(false);
    }
  }

  const handleAvatarGeniusStart = async () => {
    if (!canUseAvatarGenius || !activeChapter?.id || !currentGeniusSection) {
      pushToast({
        title: "Genius Mode unavailable",
        description: geniusUnavailableReason || "Open a section page to use Genius Mode.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const response = await startAvatarSession({
        unitId: String(activeChapter.id),
        sectionTitle: String(currentGeniusSection.title || currentGeniusSection.label),
      });
  ...
  const handleAvatarClose = async () => {
    ...
    if (avatarSessionId && avatarStatus !== "completed" && !avatarEndCalledRef.current) {
      avatarEndCalledRef.current = true;
      try {
        await endAvatarSession({ sessionId: avatarSessionId });
      } catch {}
  ...
  const handleAvatarMcqSubmit = () => {
    ...
    const answer = String(avatarMcqAnswers[avatarCurrentKey] || "").trim().toUpperCase();
    const correctAnswer = String(avatarDisplaySegment.answer || "").trim().toUpperCase();
    ...
    setAvatarMcqFeedback((prev) => ({
      ...
      [avatarCurrentKey]: {
        ...
        message: isCorrect
          ...
          : selectedExplanation || correctExplanation || `The correct answer is ${correctAnswer}.`,
      },
    }));
  };
  ...
  useEffect(() => {
    ...
    audio.play().catch(() => {
      ...
      setAvatarStatus(String(segment.type || "").toLowerCase() === "flashcard" ? "waiting_flashcard" : "paused");
    });
          ...
          <div className="workstation">
            ...
            <main className="main-viewport" onMouseUp={handleMouseUp}>
              <div className="scroll-canvas">
                ...
                {/* Floating Genius/Summarize actions (inside book sheet area) */}
                <div className="float-actions no-select">
                  {/* Left/right spread nav arrows */}
                  <button
                    type="button"
                    className="float-arrow float-arrow-left"
                    disabled={!hasPrevSpread || isFlipping}
                    onClick={(e) => {
                      e.stopPropagation();
                      if (!hasPrevSpread || isFlipping) return;
                      goSpread(-1);
                    }}
                    title="Previous page"
                  >
                  ...
                  <button
                    type="button"
                    className="float-arrow float-arrow-right"
                    disabled={!hasNextSpread || isFlipping}
                    onClick={(e) => {
                      e.stopPropagation();
                      if (!hasNextSpread || isFlipping) return;
                      goSpread(1);
                    }}
                    title="Next page"
                  >
                  ...
                  <button
                    type="button"
                    className={`float-ai-btn float-ai-genius ${!canUseAvatarGenius ? "locked" : ""}`}
                    ...
                    title={canUseAvatarGenius ? "Genius Mode" : geniusUnavailableReason}
                  >
                    <Sparkles size={14} />
                    <span>Genius</span>
                    {isAvatarStarting ? "..." : null}
                  </button>

                  {/* Summarize button */}
                  <button
                    type="button"
                    className="float-ai-btn float-ai-summarize"
                    disabled={isEnhancing}
                    onClick={(e) => {
                      e.stopPropagation();
                      const selected = persistentSelection.current;
                      if (!selected || !selected.trim()) {
                        pushToast({
                          title: "Select text to summarize",
                          description: "Highlight a passage first, then click Summarize.",
                          variant: "destructive",
                        });
                        return;
                      }
                    ...
                    >
                      ...
                      >
                        <div className="avatar-genius-head">
                          <div>
                            <span className="avatar-genius-kicker">Genius Mode</span>
                            <h3>{currentGeniusSection?.title || currentGeniusSection?.label || "Avatar Teacher"}</h3>
                          </div>
                          <button type="button" className="avatar-genius-close" onClick={handleAvatarClose} title="Close Genius Mode">
                            <X size={16} />
                          </button>
                        </div>
                        ...
                        {isAvatarStarting ? (
                          ...
                          <>
                            <div className="avatar-genius-stage">
                              <div className={`avatar-teacher-orb ${avatarStatus === "playing" ? "speaking" : ""}`}>
                                <Bot size={34} />
                              </div>
                              <div className="avatar-segment-copy">
                                <span className="avatar-segment-meta">
                                  {avatarStatus === "playing" ? <Volume2 size={13} /> : <Sparkles size={13} />}
                                  Segment {Math.min(avatarIndex + 1, Math.max(avatarSegments.length, 1))} of {Math.max(avatarSegments.length, 1)}
                                  {avatarDisplaySegment?.emotion ? ` • ${avatarDisplaySegment.emotion}` : ""}
                                </span>
                                <p>
                                  {avatarDisplaySegment?.type === "flashcard"
                                    ? avatarDisplaySegment.avatar_line || avatarDisplaySegment.front || "Let's pause for a quick check."
                                    : avatarDisplaySegment?.text || "Your avatar lesson is ready."}
                                </p>
                              </div>
                            </div>

                            {avatarGeneratingCards && (
                              <div className="avatar-inline-note">Preparing flashcards for this lesson...</div>
                            )}

                            {avatarDisplaySegment?.type === "flashcard" && (
                              <div className="avatar-flashcard">
                                <div className="avatar-flashcard-title">
                                  {avatarIsMcq ? "Quick Check" : avatarDisplaySegment.card_title || "Flashcard"}
                                </div>
                                {avatarIsMcq ? (
                                  <>
                                    <p className="avatar-mcq-question">
                                      {avatarDisplaySegment.question || "Type the correct option after the audio completes."}
                                    </p>
                                    {avatarDisplaySegment.options && (
                                      <div className="avatar-mcq-options">
                                        {Object.entries(avatarDisplaySegment.options).map(([key, value]) => (
                                          <div key={key} className="avatar-mcq-option">
                                            <strong>{key}</strong>
                                            <span>{value}</span>
                                          </div>
                                        ))}
                                    ...
                                    <div className="avatar-answer-row">
                                      <input
                                        value={avatarMcqAnswers[avatarCurrentKey] || ""}
                                        onChange={(event) =>
                                          setAvatarMcqAnswers((prev) => ({
                                            ...prev,
                                            [avatarCurrentKey]: event.target.value,
                                          }))
                                        }
                                        disabled={!avatarCanAnswerMcq}
                                        placeholder={avatarStatus === "waiting_flashcard" ? "Type A, B, C or D" : "Answer unlocks after audio"}
                                      />
                                      <button
                                        type="button"
                                        onClick={handleAvatarMcqSubmit}
                                        disabled={!avatarCanAnswerMcq || !avatarMcqAnswers[avatarCurrentKey]?.trim()}
                                      >
                                        Check
                                      </button>
                                    </div>
                                    {avatarMcqFeedback[avatarCurrentKey] && (
                                      <div className={`avatar-feedback ${avatarMcqFeedback[avatarCurrentKey].correct ? "correct" : "wrong"}`}>
                                        {avatarMcqFeedback[avatarCurrentKey].correct ? <CheckCircle2 size={18} /> : <AlertCircle size={18} />}
                                        <div>
                                          <strong>
                                            {avatarMcqFeedback[avatarCurrentKey].correct ? "Excellent" : "Review this"}
                                          </strong>
                                          <p>{avatarMcqFeedback[avatarCurrentKey].message}</p>
                                          {!avatarMcqFeedback[avatarCurrentKey].correct && avatarDisplaySegment.option_explanations && (
                                            <div className="avatar-option-explanations">
                                              {Object.entries(avatarDisplaySegment.option_explanations).map(([key, value]) => (
                                  ...
                                  <p className="avatar-flashcard-front">
                                    {avatarDisplaySegment.front || "A supporting flashcard is being prepared for this idea."}
                                  </p>
                                )}
                              </div>
                            )}
                            ...
                            {avatarClarifications.length > 0 && (
                              <div className="avatar-clarification">
                                ...
                                <button type="button" onClick={handleAvatarResume} disabled={isResumingAvatar}>
                                  {isResumingAvatar ? "Resuming..." : "Resume lesson"}
                                </button>
                              </div>
                            )}
                            ...
                            <div className="avatar-genius-actions">
                              <div className="avatar-doubt-box">
                                ...
                                <input
                                  value={avatarDoubt}
                                  onChange={(event) => setAvatarDoubt(event.target.value)}
                                  placeholder="Raise hand with a doubt"
                                />
                                <button
                                  type="button"
                                  onClick={handleAvatarRaiseHand}
                                  disabled={!avatarSessionId || !avatarDoubt.trim() || isRaisingHand}
                                >
                                  {isRaisingHand ? "Asking..." : "Ask"}
                                </button>
                              </div>

                              {avatarStatus === "paused" && !avatarClarifications.length && (
                                <button type="button" className="avatar-secondary-btn" onClick={() => setAvatarStatus("playing")}>
                                  Continue audio
                                </button>
                              )}
                              {avatarStatus === "waiting_flashcard" && !avatarIsMcq && (
                                <button type="button" className="avatar-primary-btn" onClick={advanceAvatarSegment}>
                                  Continue
                                </button>
                              )}
                              {avatarStatus === "waiting_flashcard" && avatarIsMcq && avatarMcqFeedback[avatarCurrentKey] && (
                                <button type="button" className="avatar-primary-btn" onClick={advanceAvatarSegment}>
                                  Continue
                                </button>
                              )}
                              {avatarStatus === "completed" && (
                                <button type="button" className="avatar-primary-btn" onClick={handleAvatarClose}>
                                  Done
                                </button>
                              )}
                            </div>
