...
const genRoomLink = id => `${typeof window!=="undefined"?window.location.origin:""}/seminar/join?room=${id}`;
...
function SeminarSetup({onBack,onLaunch}) {
  ...
  const leftFeatures=seminarMode==="prepare"?[
    ...
    {ic:"📋",t:"FAQ Notes",d:"AI answers saved as notes"},{ic:"🎭",t:"Demo Mode",d:"Practice with AI feedback"},
...
function PrepareWithAIRoom({config,onEnd}) {
  ...
  const [messages,setMessages]=useState([
    {from:"ai",text:`Welcome! I've analysed your topic: "${config.topic}"${config.subject?` in ${config.subject}${config.unit?`, ${config.unit}`:""}`:""}. I'm your AI coach — ask me anything via text or use the voice button 🎙️. I can help with outlines, scripts, Q&A prep, and delivery feedback. When you're ready to practice, click "Demo Mode" to start your timed presentation. You can also share your screen to present and I'll analyse your content!`}
  ]);
  ...
  useEffect(()=>{
    toast$("Click Start Demo to begin the session", "info");
    if(config.stream){
  ...
  useEffect(()=>{
    if(speech.transcript){setLiveTranscript(speech.transcript);setIsStudentSpeaking(true);}
    else if(!speech.isListening){setTimeout(()=>setIsStudentSpeaking(false),1500);}
  ...
  useEffect(()=>{
    ...
    stuckCheckRef.current=setInterval(()=>{
      ...
      if(silentFor>=12&&!aiPausedForSuggestion){
        triggerAISuggestion();
      }
    },3000);
    return()=>{if(stuckCheckRef.current)clearInterval(stuckCheckRef.current);};
  },[demoMode,demoRunning,aiPausedForSuggestion]);
  ...
  function addNote(question,answer){const n=noteCount;setNoteCount(c=>c+1);setNotes(prev=>[{id:Date.now(),n,q:question,a:answer,open:false},...prev].slice(0,20));}
  function toggleNote(id){setNotes(prev=>prev.map(n=>n.id===id?{...n,open:!n.open}:n));}
  ...
  function toggleVoiceAsk(){
    if(voiceListening){voiceAsk.stop();setVoiceListening(false);if(voiceAsk.transcript.trim()){sendAI(voiceAsk.transcript.trim());}toast$("Voice captured","info");}
    else{setVoiceListening(true);voiceAsk.start(finalText=>{voiceAsk.stop();setVoiceListening(false);sendAI(finalText);toast$("Voice sent to AI","success");});toast$("🎙️ Listening — speak your question","info");}
  }
  ...
  function enterDemoReadyState(){
    ...
    setDemoReady(true);
    toast$("🎭 Click 'Start Demo Now' to begin your presentation practice","info");
  }
  ...
  function actuallyStartDemo(){
    ...
    lastSpeechTimeRef.current=Date.now();
    const msg=`Demo mode activated! Present your seminar on "${config.topic}" as if to a live audience. I'm analysing: introduction clarity, argument structure, evidence usage, transitions, and closing impact. Speak naturally — go ahead!`;
    setMessages(m=>[...m,{from:"system",text:"🎭 Demo Mode Started — Present now"},{from:"ai",text:msg}]);
    addNote("Demo mode started","I am now evaluating: clarity, structure, evidence, transitions, and closing impact.");
    aiVoice.speak("Demo mode active. Please begin your presentation.");
    toast$("🎭 Demo started — speak freely","info");
    setPanelOpen(false);
  }
  ...
  return (
    <div className="prep-page">
      ...
      <div className="prep-bar">
        ...
        {demoMode&&<div className={`prep-pill ${demoRunning?"pp-demo":"pp-paused"}`}>{demoRunning?`🎭 DEMO ${demoTimerStr}`:`⏸ DEMO PAUSED ${demoTimerStr}`}</div>}
        ...
        <div className="prep-main-area">
              ...
              <div className="prep-ss-area">
                ...
                {timer.isPaused&&!isAiPaused&&(
                  <div className="paused-overlay">
                    ...
                    <button className="paused-resume-btn" onClick={handleResumeSession}>▶ Resume Session</button>
                  </div>
                )}
                {/* AI stuck banner */}
            ...
            <div className="prep-stage">
              <div className="prep-tiles-grid">
                ...
                <div className={`prep-tile${aiVoice.isSpeaking&&!timer.isPaused?" speaking-ai":""}`}>
                  ...
                  <div className="prep-tile-role">Powered by Claude</div>
              ...
              {demoReady&&(
                <div className="demo-ready-banner">
                  <div className="demo-ready-inner">
                    ...
                    <div className="demo-ready-desc">
                      When you tap "Start Demo Now", the timer will begin and AI will analyse your live presentation of:<br/>
                      <strong style={{color:"#5ee3b7"}}>"{config.topic}"</strong><br/>
                      Speak naturally — deliver your seminar as if to a real audience.
                    </div>
                    <button className="demo-start-btn" onClick={actuallyStartDemo}>
                      <span>🎭</span> Start Demo Now
                    </button>
          ...
          <div className="prep-ctrl-bar">
            ...
            <div className="prep-ctrl-center">
              ...
              {/* Mic */}
              <button className={`cbtn ${micOn?"on":"off"}`} onClick={()=>setMicOn(m=>!m)}>
                <span className="cbtn-ic">{micOn?"🎤":"🔇"}</span>
                {micOn&&isStudentSpeaking&&!timer.isPaused?<div className="cbtn-analyser"><SoundAnalyser active color="#5ee3b7" bars={4} size={12}/></div>:<span>{micOn?"Mute":"Mic"}</span>}
              </button>
              ...
              {!presentMode&&!isScreenSharing?(
                <button className="cbtn hi" onClick={startPresenting} disabled={timer.isPaused}>
                  <span className="cbtn-ic">🖥️</span><span>Present</span>
                </button>
              ):(
                <button className="cbtn off" onClick={stopPresenting}>
                  <span className="cbtn-ic">⏹</span><span>Stop</span>
                </button>
              )}

              {/* Demo controls — appear only after demo starts */}
              {!demoMode&&!demoReady&&(
                <button className="cbtn em" onClick={enterDemoReadyState} disabled={timer.isPaused}><span className="cbtn-ic">▶</span><span>Start Demo</span></button>
              )}
              {demoReady&&(
                <button className="cbtn em" onClick={actuallyStartDemo}><span className="cbtn-ic">▶</span><span>Start!</span></button>
              )}
              {demoMode&&demoRunning&&!isAiPaused&&(
                <>
                  <button className="cbtn pause-btn" onClick={pauseDemo}><span className="cbtn-ic">⏸</span><span>Pause</span></button>
                  <button className="cbtn rec" onClick={stopDemo}><span className="cbtn-ic">🏁</span><span>End Demo</span></button>
                </>
              )}
              {demoMode&&!demoRunning&&!isAiPaused&&(
                <>
                  <button className="cbtn em" onClick={resumeDemo} disabled={timer.isPaused}><span className="cbtn-ic">▶</span><span>Resume</span></button>
                  <button className="cbtn rec" onClick={stopDemo}><span className="cbtn-ic">🏁</span><span>Stop</span></button>
                </>
              )}
              {isAiPaused&&(
                <button className="cbtn em" onClick={handleManualResume}><span className="cbtn-ic">▶</span><span>Resume</span></button>
              )}
            ...
            <div className="prep-ctrl-right">
              ...
              <button className={`cbtn${activePanel==="actions"&&panelOpen?" hi":""}`} onClick={()=>{setActivePanel("actions");setPanelOpen(true);}}><span className="cbtn-ic">⚡</span><span>Quick</span></button>
          ...
          <div className="prep-side-panel">
            <div className="prep-panel-header">
              <div className="prep-panel-tabs">
                {[{id:"chat",ic:"🤖",lbl:"AI Chat"},{id:"notes",ic:"📋",lbl:"Notes"},{id:"actions",ic:"⚡",lbl:"Quick"}].map(t=>(
              ...
              <div style={{display:"flex",flexDirection:"column",height:"100%",minHeight:0,overflow:"hidden"}}>
                ...
                <div className="quick-prompts">
                  {["Outline","Script","Questions","Examples","Feedback"].map(qp=>(
                    <button key={qp} className="quick-p" onClick={()=>sendAI(qp)}>{qp}</button>
                  ))}
                </div>
              ...
              <div className="prep-panel-scroll">
                <div className="prep-actions-panel">
                  ...
                  {[{ic:"📋",label:"Generate Full Outline",prompt:"Generate a complete seminar outline"},
                    ...
                    <button key={i} className="prep-action-btn" onClick={()=>{sendAI(a.prompt);setActivePanel("chat");setPanelOpen(true);}}>{a.ic} {a.label}</button>
                  ))}
                  ...
                  {!demoMode&&!demoReady&&(
                    <button className="prep-action-btn primary" onClick={()=>enterDemoReadyState()}>🎭 Start Demo</button>
                  )}
                  {demoReady&&(
                    <><button className="prep-action-btn primary" onClick={actuallyStartDemo}>▶ Start Demo Now</button>
                    <button className="prep-action-btn" onClick={cancelDemoReady}>✕ Cancel Demo</button></>
                  )}
                  {demoMode&&demoRunning&&!isAiPaused&&(
                    <><button className="prep-action-btn pause-active" onClick={pauseDemo}>⏸ Pause Demo</button>
                    <button className="prep-action-btn demo-active" onClick={()=>stopDemo()}>🏁 End Demo & Get Feedback</button></>
                  )}
                  {demoMode&&!demoRunning&&!isAiPaused&&(
                    <><button className="prep-action-btn primary" onClick={resumeDemo}>▶ Resume Demo</button>
                    <button className="prep-action-btn demo-active" onClick={()=>stopDemo()}>🏁 End Demo & Get Feedback</button></>
                  )}
                  {isAiPaused&&(
...
export default function SeminarPage() {
  ...
  function handleNew(){setResult(null);setConfig(null);setScreen("setup");}
